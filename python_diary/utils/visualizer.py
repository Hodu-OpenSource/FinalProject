import numpy as np # 배열 연산, 이미지 데이터 조작, 모자이크 배열 생성
import matplotlib.cm as cm # 컬러맵을 제공하여 시각적으로 데이터를 표현
from mpl_toolkits.axes_grid1 import make_axes_locatable # 플롯 레이아웃 조정, 컬러바 위치 지정
import matplotlib.pyplot as plt # 시각화를 위한 플롯 생성, 데이터 값 표시 및 레이아웃 정렬
import numpy.ma as ma # 모자이크 배열의 특정 위치를 마스킹 처리
import cv2 # 텍스트 추가 및 이미지 처리 관련 기능
from .inference import draw_text

# 데이터 로드 및 전처리
# face.pkl과 emotions.pkl 파일에서 입력데이터(얼굴) 레이블(감정) 로드 

# 모자이크 생성 및 시각화
# make_mosaic 함수를 사용하여 이미지 데이터를 모자이크 형식으로 출력
# 감정 레이블(class_detector)을 각 이미지와 함께 표시

# CNN 모델 가중치 시각화
# 모델(simple_CNN.985-0.66.hdf5)의 첫번째 합성곱 레이어에서 학습된 필터 가중치를 추출
# 필터 가중치를 모자이크 형식으로 출력하여, 필터가 학습한 특징을 시각적으로 분석
